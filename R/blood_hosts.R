...
#' @keywords internal
#' @export
OtherBloodHosts <- function(t, y, xds_obj) {
  UseMethod("OtherBloodHosts", xds_obj$XY_interface$blood_host_obj)
}
...
blood_hosts_dynamics <- function(t, y, xds_obj){
  for(s in 1:xds_obj$nVectorSpecies){
    blood_host_pars <- xds_obj$XY_interface$blood_host_obj[[s]]$blood_host_pars
    V <- get_variables(blood_host_pars, t, y, xds_obj, s)
